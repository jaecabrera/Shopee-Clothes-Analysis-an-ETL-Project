...
# Links/Response data
input_data = pd.read_csv(f'./extract_exports/{TODAY}_links(BASE).csv')
...
# Review Process
crawl_review = CrawlerRunner(
    settings={'FEEDS': {pathlib.PureWindowsPath("""
    ./extract_exports/{TODAY}s_review(DIRTY).csv
    """): {'format': 'json'}}}
)

# Shop Profile Process
shop_review = CrawlerRunner(
    settings={'FEEDS': {pathlib.PureWindowsPath("""
    ./extract_exports/{TODAY}s_link(DIRTY).csv
    """): {'format': 'json'}}}
)
...
def crawl():
    yield crawl_review.crawl(ReviewsSpider)
    yield shop_review.crawl(ShopSpider)
    reactor.stop()
